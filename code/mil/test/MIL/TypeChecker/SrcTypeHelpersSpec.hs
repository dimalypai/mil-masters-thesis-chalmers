...
spec = do
  ...
  describe "srcTypeToTypeWithTypeVars" $ do
    it "transforms a type variable correctly" $
      let srcType = SrcTyTypeCon (TypeName "A")
          expT = mkTypeVar "A" in
      fst <$> runTypeCheckM (srcTypeToTypeWithTypeVars (Set.fromList [TypeVar "A"]) srcType) (initTypeEnv $ repeat defaultMonadError)
        `shouldBe` Right expT
  ...
  describe "srcTypeToTypeWithTypeVarsOfKind" $ do
    it "performs kind checking for function type" $
      ...
      fst <$> runTypeCheckM (srcTypeToTypeWithTypeVarsOfKind Set.empty (mkKind 1) srcType) (initTypeEnv $ repeat defaultMonadError)
...
successCaseWithSetup setup srcType expT =
  fst <$> runTypeCheckM (setup >> srcTypeToType srcType) (initTypeEnv $ repeat defaultMonadError)
    `shouldBe` Right expT
