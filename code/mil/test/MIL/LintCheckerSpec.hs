...
successCase baseName = do
  ...
  case typeCheck srcProgram (repeat defaultMonadError) of
    Right (tyProgram, typeEnv) ->
      case lintCheck tyProgram (repeat defaultMonadError) of
...
failureCase baseName = do
  ...
  let srcProgram = parseMil input
  let Right (tyProgram, _) = typeCheck srcProgram (repeat defaultMonadError)
  let Left err = lintCheck tyProgram (repeat defaultMonadError)
